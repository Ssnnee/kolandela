    ...
    <View style={{ paddingHorizontal: 24, marginBottom: 20, gap: 12 }}>
      ...
      <View style={{ backgroundColor: isDeficit ? redColor : primaryColor, borderRadius: 20, padding: 20 }}>
        <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11, fontWeight: '500', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 4 }}>
          {isDeficit ? t('tabs.home.overspent') : t('tabs.home.remaining')}
        </Text>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 6 }}>
          {isDeficit ? (
            <View style={{ backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 100, paddingHorizontal: 10, paddingVertical: 3 }}>
              <Text style={{ color: 'white', fontSize: 11, fontWeight: '600' }}>{t('tabs.home.deficit')}</Text>
            </View>
          ) : (
            <View style={{ backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 100, paddingHorizontal: 10, paddingVertical: 3 }}>
              <Text style={{ color: 'white', fontSize: 11, fontWeight: '600' }}>{t('tabs.home.saved', { rate: savingsRate })}</Text>
            </View>
          )}
          <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11 }}>{t('tabs.home.ofIncome')}</Text>
        </View>
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 12 }}>
        {[
          { label: t('tabs.home.income'), value: income, color: violetColor },
          { label: t('tabs.home.expenses'), value: expenses, color: primaryColor },
        ].map((item) => (
